...
export const ObjectiveSetting: React.FC<{ onBack: () => void }> = ({ onBack }) => {
  ...
  const handleSave = async () => {
    ...
    const configsToSave: ExpensesObjectiveConfigFront[] = categories.map((cat) => ({
      tempId: crypto.randomUUID(),
      userId: user.id,
      categoryId: cat.id,
      amount: amountInputs[cat.id] ? parseInt(amountInputs[cat.id], 10) : null,
      day: 1,
    }));

    try {
      setSaving(true);
      await ExpensesObjectiveConfigRepository.saveConfigs(user.id, configsToSave);
      alert('目標金額を保存しました！');
      onBack();
    } catch (error) {
  ...
  return (
    <Box sx={{ p: 1, bgcolor: APP_COLORS.background, minHeight: '100vh' }}>
      ...
      >
        <TrackChanges sx={{ color: APP_COLORS.mainGreen, mr: 1, fontSize: 20 }} />
        <Typography variant="body2" sx={{ color: APP_COLORS.textPrimary, opacity: 0.8 }}>
      ...
      >
        ...
        <Typography variant="h5" sx={{ fontWeight: 'bold', color: APP_COLORS.mainGreen }}>
